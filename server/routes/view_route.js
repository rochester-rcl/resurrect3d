module.exports = (app, upload, conn, Grid, router) => {
  ...
  router.route("/api/users/login").post(user.login, user.onLogin);

  router.route("/api/users/logout").get(user.logout);

  router.route("/api/users/authenticate").get(user.authenticateClient);
  ...
  router.route("/api/users/:id").delete(user.authenticateServer, user.delete);

  router.route("/api/users/verify/:token").get(user.verify);
};
