...
export default function loadBloomPass(threeInstance: Object): typeof Promise {
  return new Promise((resolve, reject) => {
    threeInstance.BloomPass = function (
      strength,
      kernelSize,
      sigma,
      resolution
      ...
      this.renderTargetX = new threeInstance.WebGLRenderTarget(
        resolution,
        resolution,
        pars
      );
      this.renderTargetX.texture.name = "BloomPass.x";
      this.renderTargetY = new threeInstance.WebGLRenderTarget(
        resolution,
        resolution,
        pars
      );
      ...
      if (threeInstance.CopyShader === undefined)
        console.error(
          "threeInstance.BloomPass relies on threeInstance.CopyShader"
        );

      var copyShader = threeInstance.CopyShader;

      this.copyUniforms = threeInstance.UniformsUtils.clone(
        copyShader.uniforms
      );
      ...
      if (threeInstance.ConvolutionShader === undefined)
        console.error(
          "threeInstance.BloomPass relies on threeInstance.ConvolutionShader"
        );

      var convolutionShader = threeInstance.ConvolutionShader;

      this.convolutionUniforms = threeInstance.UniformsUtils.clone(
        convolutionShader.uniforms
      );

      this.convolutionUniforms["uImageIncrement"].value =
        threeInstance.BloomPass.blurX;
      this.convolutionUniforms[
        "cKernel"
      ].value = threeInstance.ConvolutionShader.buildKernel(sigma);
    ...
    threeInstance.BloomPass.prototype = Object.assign(
      Object.create(threeInstance.Pass.prototype),
      {
        ...
        render: function (
          ...
          if (maskActive) ctx.disable(ctx.STENCIL_TEST);
          ...
          this.convolutionUniforms[
            "tDiffuse"
          ].value = this.renderTargetX.texture;
          this.convolutionUniforms["uImageIncrement"].value =
            threeInstance.BloomPass.blurY;
          ...
          if (maskActive) ctx.enable(ctx.STENCIL_TEST);
