  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      for (let i = 0; i < children.length; i++) {
        ...
        const completedNormalMaps = new Set([]);
        const materials =
        ...
        for (let j = 0; j < materials.length; j++) {
          ...
          if (
            material.map &&
            !completedNormalMaps.has(material.uuid) &&
            material.map.image &&
            !material.normalMap
          ) {
            ...
            for (let y = 0; y < h; y++) {
              for (let x = 0; x < w; x++) {
                ...
                const norm = new THREE.Vector3(u, v, zScale)
                  .normalize()
                  .toArray();
            ...
            const tex = cloneTexture(material.map);
            tex.image.src = outCanvas.toDataURL();
            material.normalMap = tex;
            completedNormalMaps.add(material.uuid);
          }
        }
      }
      resolve(mesh);
...
export function solveQuartic(c: Array, _s: Array) {
  ...
  const r =
    (-3.0 / 256) * sq_a * sq_a + (1.0 / 16) * sq_a * b - (1.0 / 4) * a * _c + d;
...
export function computeMaxOnCircle(a, _x, _y) {
  ...
  if (
    Math.abs(db0) < ZERO_TOL &&
    Math.abs(db1) < ZERO_TOL &&
    Math.abs(db2) < ZERO_TOL &&
    Math.abs(db3) < ZERO_TOL &&
    Math.abs(db4) < ZERO_TOL
  ) {
    return [0.0, 1.0, 1];
  }
  ...
  if (nroots === 1) {
    ...
    for (let i = 0; i < nroots; i++) {
      ...
      vals[i] =
        a[0] * u * u + a[1] * v * v + a[2] * u * v + a[3] * u + a[4] * v + a[5];
