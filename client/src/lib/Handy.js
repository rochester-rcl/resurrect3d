		...
		for( let 
			...
			if( i === 0 ){
				...
				if( hand.livePoseData.jointPositions.length === 0 ||
					( 
						hand.livePoseData.jointPositions[ 1 ][ 0 ] === 0 &&
						hand.livePoseData.jointPositions[ 1 ][ 1 ] === 0 &&
						hand.livePoseData.jointPositions[ 1 ][ 2 ] === 0
					)){
					return
				}
					...
					if( hand.lastSearchResult && hand.lastSearchResult.pose ){
						...
						.forEach( function( poseName ){
							...
							hand.dispatchEvent({
								...
								message:  (hand.handedness || 'right_matankb').toUpperCase() +
	...
	compareAllTo: function( inputPose ){
		...
		.reduce( function( list, pose ){ 
			...
			return list.concat({ 
				...
				name: pose.name, 
				distance: Handy.hands.left.comparePoses( 'Fist', pose.name )
			})
